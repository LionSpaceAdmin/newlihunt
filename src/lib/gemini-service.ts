...
const analysisResponseSchema = {
  ...
  properties: {
    summary: {
      type: Type.STRING,
      description:
        'A natural, helpful explanation of the analysis in a conversational tone. Include a summary of findings, key concerns, actionable recommendations, and educational insights about scam tactics.',
    },
    analysisData: {
      type: Type.OBJECT,
      properties: {
        ...
        reasoning: {
          type: Type.STRING,
          description: 'Detailed explanation of the analysis and scoring',
        },
        debiasingStatus: {
          type: Type.OBJECT,
          properties: {
...
export const analyzeScam = async (
  ...
  const contents: Part[] = [
    { text: `Analyze the following content for impersonation scams. URL/Handle: ${text}` },
  ];
...
export const getChatInstance = (): Chat => {
  if (!chatInstance) {
    ...
    chatInstance = ai.chats.create({
      ...
      config: {
        systemInstruction:
          'You are a helpful assistant for the Scam Hunt Platform. Answer user questions concisely and clearly.',
      },
    });
  }
  return chatInstance;
