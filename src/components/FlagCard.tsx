...
const textContent = {
  en: {
    ...
    fake: 'Fake/Scam Signal',
  },
  he: {
    points: 'נקודות',
    expand: 'הצג פרטים',
    collapse: 'הסתר פרטים',
    legitimate: 'סימן לגיטימי',
    suspicious: 'סימן חשוד',
    fake: 'סימן מזויף/הונאה',
  },
};
...
const FlagCard: React.FC<FlagCardProps> = ({ rule, lang = 'en' }) => {
  ...
  // Get category-specific styling (LionsOfZion v1.3)
  const getCategoryStyle = () => {
    if (rule.category === 'Legitimate') {
      return {
        bg: 'bg-green-900/20',
    ...
    } else if (rule.category === 'Suspicious') {
      return {
        bg: 'bg-yellow-900/20',
        border: 'border-yellow-600/30',
        text: 'text-yellow-300',
  ...
  const cardStyle = categoryStyle || colors;

  return (
    <div
      className={`${cardStyle.bg} ${cardStyle.border} border rounded-lg p-4 transition-all duration-200 hover:shadow-lg`}
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-2">
            <div className="flex-shrink-0">{colors.icon}</div>
            <h3 className={`font-semibold ${cardStyle.text}`}>{rule.name}</h3>
            <span
            ...
            {rule.category && (
              <span
                className={`px-2 py-1 text-xs rounded-full ${cardStyle.bg} ${cardStyle.text} border ${cardStyle.border} font-medium`}
              >
                {categoryStyle?.label}
              </span>
            )}
